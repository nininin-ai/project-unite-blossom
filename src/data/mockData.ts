// ── New hierarchical structure: Asset → Lease → Floor → Lot ──
...
export interface Lease {
  ...
  startDate: string;
  endDate: string;
  triennialDate: string; // only relevant for 3/6/9 or 6/9
  leaseType: string;
  deposit: number;
  currentRent: number;
  rentInputMode: "annual" | "monthly";
...
export interface Asset {
  ...
  annualRent: number; // computed from leases
  yield: number;
  charges: Charge[];
  leases: Lease[];
}
...
/** Flat view: one row per lease×floor×lot (for table display) */
export function getAssetLeases(asset: Asset): (Lease & { floorName: string; floorLevel: number; lotName: string; lotSurface: number; lotType: string })[] {
  return (asset.leases ?? []).flatMap(lease =>
    (lease.floors ?? []).flatMap(f =>
      f.lots.map(l => ({
        ...lease,
        floorName: f.name,
        floorLevel: f.level,
...
/** Get the annual rent for a single lease (handles monthly mode) */
export function getLeaseAnnualRent(lease: Lease): number {
  return lease.rentInputMode === "monthly" ? lease.currentRent * 12 : lease.currentRent;
}

/** Compute total annual rent from all leases */
export function getAssetAnnualRent(asset: Asset): number {
  return (asset.leases ?? []).reduce((s, l) => s + getLeaseAnnualRent(l), 0);
}

/** Total surface occupied by leases */
export function getAssetOccupiedSurface(asset: Asset): number {
  return (asset.leases ?? []).reduce((s, l) =>
    s + (l.floors ?? []).reduce((fs, f) =>
      fs + f.lots.reduce((ls, lot) => ls + lot.surface, 0), 0), 0);
}

/** Vacant surface = total - occupied */
export function getAssetVacantSurface(asset: Asset): number {
  return Math.max(0, asset.totalSurface - getAssetOccupiedSurface(asset));
}
...
/** Check if lease type requires trienniale */
export function leaseHasTrienniale(leaseType: string): boolean {
  return leaseType === "3/6/9" || leaseType === "6/9";
}
...
const uid = () => crypto.randomUUID ? crypto.randomUUID() : `id-${Math.random().toString(36).slice(2, 8)}`;

const mkLease = (p: Partial<Lease> & { tenantName: string; currentRent: number; floors: Floor[] }): Lease => ({
  id: uid(),
  isParticulier: false,
  tenantSiren: "",
  startDate: "",
...
export const mockAssets: Asset[] = [
  {
    ...
    leases: [
      mkLease({ id: "t3", tenantName: "StartupFlow SAS", tenantSiren: "912345678", startDate: "2023-01-01", endDate: "2025-12-31", triennialDate: "", leaseType: "Dérogatoire", deposit: 22000, currentRent: 130000, index: "ILC", indexQuarter: "T2", indexYear: 2024, accompaniment: "Loyers progressifs", chargesManagement: "Forfaitaire", unpaid: true, unpaidAmount: 12500, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f1-8", name: "Étage 8", level: 8, lots: [{ id: "lot-8a", name: "Lot 8A", surface: 420, type: "Bureau" }] },
      ] }),
      mkLease({ id: "t2", tenantName: "Mercer France", tenantSiren: "390589455", startDate: "2021-06-01", endDate: "2027-05-31", triennialDate: "2024-06-01", deposit: 62000, currentRent: 310000, indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f1-10", name: "Étage 10", level: 10, lots: [{ id: "lot-10a", name: "Lot 10A", surface: 1050, type: "Bureau" }] },
      ] }),
      mkLease({ id: "t4", tenantName: "LegalTech Corp", tenantSiren: "823456789", startDate: "2022-03-01", endDate: "2031-02-28", triennialDate: "2025-03-01", deposit: 75000, currentRent: 400000, indexQuarter: "T3", indexYear: 2024, accompaniment: "Travaux preneur 50k€", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f1-11", name: "Étage 11", level: 11, lots: [{ id: "lot-11a", name: "Lot 11A", surface: 950, type: "Bureau" }] },
      ] }),
      mkLease({ id: "t1", tenantName: "Deloitte France", tenantSiren: "434209797", startDate: "2020-01-01", endDate: "2028-12-31", triennialDate: "2026-01-01", deposit: 85000, currentRent: 420000, indexQuarter: "T3", indexYear: 2024, accompaniment: "Franchise 3 mois", chargesManagement: "Forfaitaire", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f1-12", name: "Étage 12", level: 12, lots: [{ id: "lot-12a", name: "Lot 12A", surface: 1400, type: "Bureau" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ id: "t5", tenantName: "Boulanger", tenantSiren: "347384570", startDate: "2021-09-01", endDate: "2030-08-31", triennialDate: "2027-09-01", deposit: 45000, currentRent: 280000, index: "ILC", indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f2-0", name: "RDC", level: 0, lots: [{ id: "lot-2a", name: "Commerce A", surface: 1200, type: "Commerce" }] },
      ] }),
      mkLease({ id: "t6", tenantName: "Pharmacie du Centre", tenantSiren: "451234567", startDate: "2022-01-01", endDate: "2031-12-31", triennialDate: "2025-01-01", deposit: 35000, currentRent: 180000, index: "ILC", indexQuarter: "T2", indexYear: 2024, accompaniment: "Franchise 2 mois", chargesManagement: "Forfaitaire", isVatApplicable: false, floors: [
        { id: "f2-0b", name: "RDC", level: 0, lots: [{ id: "lot-2b", name: "Commerce B", surface: 600, type: "Commerce" }] },
      ] }),
      mkLease({ id: "t7", tenantName: "Fitness Park", tenantSiren: "327126827", startDate: "2023-06-01", endDate: "2032-05-31", triennialDate: "2026-06-01", deposit: 25000, currentRent: 160000, index: "ILC", indexQuarter: "T3", indexYear: 2024, unpaid: true, unpaidAmount: 8200, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f2-1", name: "Étage 1", level: 1, lots: [{ id: "lot-2d", name: "Sport A", surface: 400, type: "Commerce" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ tenantName: "Particulier - Petit", isParticulier: true, startDate: "2022-01-01", endDate: "2025-12-31", leaseType: "Bail civil", deposit: 2500, currentRent: 16000, index: "Aucun", indexQuarter: "T1", indexYear: 2024, chargesManagement: "Provisions", isVatApplicable: false, floors: [
        { id: "f3-1a", name: "Étage 1", level: 1, lots: [{ id: "lot-3b", name: "Appt 1A", surface: 150, type: "Résidentiel" }] },
      ] }),
      mkLease({ tenantName: "Particulier - Blanc", isParticulier: true, startDate: "2023-06-01", endDate: "2026-05-31", leaseType: "Bail civil", deposit: 2800, currentRent: 17000, index: "Aucun", indexQuarter: "T1", indexYear: 2024, chargesManagement: "Provisions", isVatApplicable: false, floors: [
        { id: "f3-1b", name: "Étage 1", level: 1, lots: [{ id: "lot-3c", name: "Appt 1B", surface: 150, type: "Résidentiel" }] },
      ] }),
      mkLease({ id: "t8", tenantName: "Particulier - Dupont", isParticulier: true, startDate: "2021-01-01", endDate: "2024-12-31", leaseType: "Bail civil", deposit: 3200, currentRent: 18000, index: "Aucun", indexQuarter: "T3", indexYear: 2024, chargesManagement: "Provisions", isVatApplicable: false, floors: [
        { id: "f3-2a", name: "Étage 2", level: 2, lots: [{ id: "lot-3d", name: "Appt 2A", surface: 75, type: "Résidentiel" }] },
      ] }),
      mkLease({ tenantName: "Particulier - Leroy", isParticulier: true, startDate: "2020-09-01", endDate: "2026-08-31", leaseType: "Bail civil", deposit: 4500, currentRent: 30000, index: "Aucun", indexQuarter: "T1", indexYear: 2024, chargesManagement: "Provisions", isVatApplicable: false, floors: [
        { id: "f3-2b", name: "Étage 2", level: 2, lots: [{ id: "lot-3e", name: "Appt 2B", surface: 275, type: "Résidentiel" }] },
      ] }),
      mkLease({ id: "t9", tenantName: "Particulier - Martin", isParticulier: true, startDate: "2022-06-01", endDate: "2025-05-31", leaseType: "Bail civil", deposit: 4100, currentRent: 24000, index: "Aucun", indexQuarter: "T3", indexYear: 2024, chargesManagement: "Provisions", unpaid: true, unpaidAmount: 3600, isVatApplicable: false, floors: [
        { id: "f3-3a", name: "Étage 3", level: 3, lots: [{ id: "lot-3f", name: "Appt 3A", surface: 95, type: "Résidentiel" }] },
      ] }),
      mkLease({ tenantName: "Particulier - Garcia", isParticulier: true, startDate: "2021-03-01", endDate: "2027-02-28", leaseType: "Bail civil", deposit: 5000, currentRent: 28000, index: "Aucun", indexQuarter: "T1", indexYear: 2024, chargesManagement: "Provisions", isVatApplicable: false, floors: [
        { id: "f3-3b", name: "Étage 3", level: 3, lots: [{ id: "lot-3g", name: "Appt 3B", surface: 255, type: "Résidentiel" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ tenantName: "Services communs", startDate: "2019-01-01", endDate: "2028-12-31", triennialDate: "2025-01-01", deposit: 0, currentRent: 0, chargesManagement: "Forfaitaire", isVatApplicable: false, floors: [
        { id: "f4-0", name: "RDC", level: 0, lots: [{ id: "lot-4a", name: "Accueil", surface: 500, type: "Services" }] },
      ] }),
      mkLease({ id: "t10", tenantName: "SAP France", tenantSiren: "379821994", startDate: "2019-01-01", endDate: "2028-12-31", triennialDate: "2025-01-01", deposit: 100000, currentRent: 1200000, indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f4-1", name: "Étage 1", level: 1, lots: [
          { id: "lot-4b", name: "Plateau 1A", surface: 2000, type: "Bureau" },
          { id: "lot-4c", name: "Plateau 1B", surface: 2000, type: "Bureau" },
        ] },
      ] }),
      mkLease({ id: "t11", tenantName: "DataViz SAS", tenantSiren: "534567890", startDate: "2023-03-01", endDate: "2026-02-28", triennialDate: "", leaseType: "Dérogatoire", deposit: 45000, currentRent: 280000, indexQuarter: "T2", indexYear: 2024, accompaniment: "Franchise 6 mois", chargesManagement: "Forfaitaire", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f4-2", name: "Étage 2", level: 2, lots: [{ id: "lot-4d", name: "Plateau 2A", surface: 800, type: "Bureau" }] },
      ] }),
      mkLease({ id: "t12", tenantName: "BioTech Lab", tenantSiren: "645678901", startDate: "2022-09-01", endDate: "2025-08-31", triennialDate: "2025-09-01", deposit: 30000, currentRent: 270000, indexQuarter: "T3", indexYear: 2024, accompaniment: "Travaux bailleur 80k€", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f4-3", name: "Étage 3", level: 3, lots: [{ id: "lot-4e", name: "Labo 3A", surface: 500, type: "Laboratoire" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ id: "t13", tenantName: "Geodis Logistics", tenantSiren: "383474825", startDate: "2020-04-01", endDate: "2032-03-31", triennialDate: "2026-04-01", deposit: 180000, currentRent: 720000, indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f5-0a", name: "RDC", level: 0, lots: [{ id: "lot-5a", name: "Cellule A", surface: 8000, type: "Entrepôt" }] },
      ] }),
      mkLease({ id: "t14", tenantName: "Amazon France Log", tenantSiren: "487482018", startDate: "2021-01-01", endDate: "2030-12-31", triennialDate: "2027-01-01", deposit: 95000, currentRent: 380000, indexQuarter: "T3", indexYear: 2024, chargesManagement: "Triple net", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f5-0b", name: "RDC", level: 0, lots: [{ id: "lot-5b", name: "Cellule B", surface: 4000, type: "Entrepôt" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ id: "t15", tenantName: "Sephora", tenantSiren: "393712286", startDate: "2018-01-01", endDate: "2030-12-31", triennialDate: "2027-01-01", deposit: 130000, currentRent: 520000, index: "ILC", indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f6-0a", name: "RDC", level: 0, lots: [{ id: "lot-6a", name: "Boutique A", surface: 550, type: "Commerce" }] },
      ] }),
      mkLease({ id: "t16", tenantName: "Maison Ladurée", tenantSiren: "775670284", startDate: "2019-06-01", endDate: "2028-05-31", triennialDate: "2025-06-01", deposit: 45000, currentRent: 260000, index: "ILC", indexQuarter: "T3", indexYear: 2024, chargesManagement: "Forfaitaire", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f6-0b", name: "RDC", level: 0, lots: [{ id: "lot-6b", name: "Boutique B", surface: 300, type: "Commerce" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ id: "t17", tenantName: "CMA CGM Services", tenantSiren: "562024422", startDate: "2022-04-01", endDate: "2031-03-31", triennialDate: "2025-04-01", deposit: 70000, currentRent: 380000, indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f7-1", name: "Étage 1", level: 1, lots: [{ id: "lot-7b", name: "Plateau 1", surface: 1600, type: "Bureau" }] },
      ] }),
      mkLease({ id: "t18", tenantName: "Startup Medtech", tenantSiren: "923456781", startDate: "2023-09-01", endDate: "2026-08-31", triennialDate: "", leaseType: "Dérogatoire", deposit: 15000, currentRent: 200000, indexQuarter: "T2", indexYear: 2024, accompaniment: "Franchise 3 mois", chargesManagement: "Forfaitaire", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f7-2", name: "Étage 2", level: 2, lots: [{ id: "lot-7c", name: "Plateau 2", surface: 800, type: "Bureau" }] },
      ] }),
    ],
  },
  {
    ...
    leases: [
      mkLease({ id: "t19", tenantName: "Decathlon", tenantSiren: "306138900", startDate: "2020-01-01", endDate: "2029-12-31", triennialDate: "2026-01-01", deposit: 75000, currentRent: 420000, index: "ILC", indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f8-0a", name: "RDC", level: 0, lots: [{ id: "lot-8a2", name: "Cellule A", surface: 2500, type: "Commerce" }] },
      ] }),
      mkLease({ id: "t20", tenantName: "Cultura", tenantSiren: "410383626", startDate: "2021-03-01", endDate: "2030-02-28", triennialDate: "2027-03-01", deposit: 40000, currentRent: 250000, index: "ILC", indexQuarter: "T3", indexYear: 2024, isVatApplicable: true, vatRate: 20, floors: [
        { id: "f8-0b", name: "RDC", level: 0, lots: [{ id: "lot-8b2", name: "Cellule B", surface: 1200, type: "Commerce" }] },
      ] }),
      mkLease({ id: "t21", tenantName: "Action", tenantSiren: "521809498", startDate: "2023-01-01", endDate: "2032-12-31", triennialDate: "2026-01-01", deposit: 20000, currentRent: 150000, index: "ILC", indexQuarter: "T2", indexYear: 2024, accompaniment: "Franchise 1 mois", chargesManagement: "Forfaitaire", isVatApplicable: true, vatRate: 20, floors: [
        { id: "f8-0c", name: "RDC", level: 0, lots: [{ id: "lot-8c2", name: "Cellule C", surface: 850, type: "Commerce" }] },
      ] }),
    ],
  },
];
...
export const mockOpportunities: NewOpportunity[] = [
  {
    id: "opp1",
    code: "EQ-2024-001",
    name: "Immeuble Haussmann Opéra",
    assetType: "Bureau",
    city: "Paris 9e",
    ...
    receptionDate: "2024-03-03",
    imageUrl: "",
    keyFacts: { constructionYear: 1890, occupancyRate: 87.5, walt: 5.2, walb: 3.8, mainTenant: "Deloitte France", indexation: "ILAT T3 2024" },
    tenants: [
      { name: "Deloitte France", startDate: "2020-01-01", endDate: "2028-12-31", leaseType: "3/6/9", annualRent: 420000, indexation: "ILAT", walt: 4.8, walb: 1.8 },
      { name: "StartupFlow SAS", startDate: "2023-01-01", endDate: "2025-12-31", leaseType: "Dérogatoire", annualRent: 130000, indexation: "ILC", walt: 1.8, walb: 0.8 },
      { name: "Mercer France", startDate: "2021-06-01", endDate: "2027-05-31", leaseType: "3/6/9", annualRent: 310000, indexation: "ILAT", walt: 3.2, walb: 0.2 },
    ],
    investment: { acquisitionPrice: 14500000, estimatedFees: 1087500, totalInvested: 15587500, capexHistory: [{ year: "2021", amount: 250000, description: "Rénovation hall d'entrée" }], capexProjection: [{ year: "2025", amount: 180000, description: "Mise aux normes PMR" }] },
    triDetail: { investmentSummary: 15587500, capexHistorique: 250000, capexProjection: 180000, projectedRents: 9860000, netCashflow: 7200000, exitValueHypothesis: 16000000 },
    aiSummary: "Immeuble haussmannien de premier plan situé au cœur du quartier Opéra avec un profil locatif diversifié. Le rendement de 6.8% est supérieur à la moyenne du marché parisien pour cette catégorie d'actif.",
    strengths: ["Emplacement prime", "Locataires de qualité", "Rendement supérieur au marché"],
    weaknesses: ["Vacance partielle", "Travaux PMR à prévoir", "Bail court StartupFlow"],
    analysisDecision: "Actif éligible – rendement attractif et emplacement sécurisé. Recommandation : poursuivre en due diligence avec focus sur l'état technique et le renouvellement StartupFlow.",
    analysisDecisionStatus: "Positif",
    portfolioImpact: "Positif",
    portfolioReasons: ["Renforce l'exposition bureau Paris prime", "Rendement supérieur à la cible fonds", "Diversification locative"],
    globalSummary: "Opportunité d'investissement solide avec un profil risque/rendement favorable. L'actif s'intègre bien dans la stratégie du fonds avec un rendement supérieur à la cible.",
  },
];
