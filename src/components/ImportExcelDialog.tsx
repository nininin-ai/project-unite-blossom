...
const ImportExcelDialog = () => {
  ...
  const handleConfirmImport = () => {
    ...
    importMutation.mutate(parseResult.assets, {
    ...
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
      <DialogTrigger asChild>
        <Button size="sm" variant="outline" className="gap-1.5">
          <Upload className="h-4 w-4" /> Importer des actifs
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Importer des actifs via Excel</DialogTitle></DialogHeader>
        {!file ? (
          <div
            ...
            {parseResult && (
              <>
                <div className="flex items-start gap-2 p-3 rounded-lg bg-accent/10 border border-accent/20">
                  ...
                  <p className="text-xs text-accent">{parseResult.count} actif(s) détecté(s)</p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={reset} className="flex-1">Annuler</Button>
                  <Button size="sm" onClick={handleConfirmImport} disabled={importMutation.isPending} className="flex-1">
                    {importMutation.isPending && <Loader2 className="h-4 w-4 animate-spin mr-1.5" />}
