...
const ExcelImport = ({ onImport }: ExcelImportProps) => {
  ...
  const processFile = useCallback((f: File) => {
    setFile(f); setError(null); setParseResult(null);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (assets.length === 0) { setError("Aucun actif reconnu."); return; }
  ...
  const handleConfirmImport = () => { if (!parseResult) return; onImport(parseResult.assets); toast.success(`${parseResult.count} actif(s) importé(s)`); setFile(null); setParseResult(null); };
  const reset = () => { setFile(null); setParseResult(null); setError(null); };

  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="kpi-card">
      <div className="flex items-center justify-between mb-4">
        <div><h2 className="text-lg font-semibold text-foreground">Importer des actifs</h2><p className="text-xs text-muted-foreground mt-0.5">Importez un fichier Excel pour ajouter des actifs</p></div>
        <FileSpreadsheet className="h-5 w-5 text-accent" />
      </div>
      {!file ? (
        <div onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop}
          className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer ${isDragging ? "border-accent bg-accent/5" : "border-border hover:border-accent/40"}`}
          onClick={() => inputRef.current?.click()}>
          <Upload className="h-8 w-8 text-muted-foreground mx-auto mb-3" />
          <p className="text-sm font-medium text-foreground">Glissez-déposez votre fichier Excel ici</p>
          <p className="text-xs text-muted-foreground mt-1">Formats supportés : .xlsx, .xls</p>
          <Button variant="default" size="sm" className="mt-4">Parcourir les fichiers</Button>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-medium text-foreground truncate">{file.name}</p><p className="text-xs text-muted-foreground">{(file.size / 1024).toFixed(0)} Ko</p></div>
            <button onClick={reset} className="text-muted-foreground hover:text-foreground transition-colors"><X className="h-4 w-4" /></button>
          </div>
          {error && <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/20"><AlertCircle className="h-4 w-4 text-destructive shrink-0 mt-0.5" /><p className="text-xs text-destructive">{error}</p></div>}
          {parseResult && (
            <>
              <div className="flex items-start gap-2 p-3 rounded-lg bg-[hsl(var(--success)/0.1)] border border-[hsl(var(--success)/0.2)]">
                <CheckCircle2 className="h-4 w-4 text-[hsl(var(--success))] shrink-0 mt-0.5" /><p className="text-xs text-[hsl(var(--success))]">{parseResult.count} actif(s) détecté(s)</p>
              </div>
              <div className="flex gap-2"><Button variant="outline" size="sm" onClick={reset} className="flex-1">Annuler</Button><Button size="sm" onClick={handleConfirmImport} className="flex-1">Importer {parseResult.count} actif(s)</Button></div>
            </>
          )}
        </div>
      )}
